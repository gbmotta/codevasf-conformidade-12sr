...
| `conformidade/loaders.py` | ZIP/pasta, PDF/DOCX/imagem, OCR Tesseract `por` |
...
- Se o texto nativo por página amostrada for **&lt; 40 caracteres** → aciona OCR
- Idioma Tesseract: **`por`**
- DPI renderização: **200**
- Máximo de páginas OCR por arquivo: **20** (evita travar em anexos grandes)
- Dependências: **PyMuPDF** + **pytesseract** + **Pillow** + binário **tesseract-ocr** (+ `tesseract-ocr-por`)

### Instalação do Tesseract

| Ambiente | Como |
|----------|------|
| Docker intranet | Já no `Dockerfile` (`tesseract-ocr`, `tesseract-ocr-por`) |
...
`ocr_available()` expõe na UI se OCR está utilizável.
...
| OCR indisponível | Tesseract + `por`; `TESSERACT_CMD` no Windows |
